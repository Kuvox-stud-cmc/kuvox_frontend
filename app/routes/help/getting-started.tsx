...
/* ── Sidebar navigation shared data ────────────────────────────────────── */

const SIDEBAR_LINKS = [
  { to: "/help/getting-started", label: "Getting Started", icon: "rocket_launch", active: true },
  ...
  return (
    <div className="w-full max-w-7xl mx-auto">
      ...
      <nav className="mb-8 flex items-center gap-2 text-body-sm text-on-surface-variant animate-fade-in-up">
        ...
        <span className="text-on-surface font-medium">Getting Started</span>
      </nav>
          ...
          <nav className="lg:sticky lg:top-24 space-y-1">
            {SIDEBAR_LINKS.map((link) => (
              <Link
                key={link.to}
                to={link.to}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-body-sm transition-colors duration-200 ${
