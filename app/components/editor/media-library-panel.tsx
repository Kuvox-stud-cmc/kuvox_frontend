...
function MockLibraryPanelContent({ tab, onClose }: { tab: string; onClose: () => void }) {
  ...
  const handleItemClick = (itemName: string) => {
    if (tab === "text") {
      ...
      dispatch(toastShown(itemName === "None" ? "Effect removed" : `Effect applied: ${itemName}`));
      ...
      dispatch(toastShown(itemName === "None" ? "Transition removed" : `Transition selected: ${itemName}`));
